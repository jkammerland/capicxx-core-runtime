...
#error                                                                         \
    "Only <CommonAPI/CommonAPI.hpp> can be included directly, this file may disappear or change contents."
#endif
...
#define DEINITIALIZER(f)                                                       \
  static void __cdecl f(void);                                                 \
  static void _##f##_wrapper(void) { atexit(f); }                              \
 ...
 * The ClientId is supposed to be added by the middleware and can be compared
 * using the == operator.
 */
class ClientId {
public:
  ...
  virtual std::string getEnv() const { return ""; }
  virtual std::string getHostAddress() const { return ""; }
};

template <typename... Args_> struct SelectiveBroadcastFunctorHelper {
  typedef std::function<void(Args_...)> SelectiveBroadcastFunctor;
};

typedef std::unordered_set<std::shared_ptr<CommonAPI::ClientId>,
                           SharedPointerClientIdContentHash,
                           SharedPointerClientIdContentEqual>
    ClientIdList;

template <typename EnumType_> class EnumHasher {
public:
